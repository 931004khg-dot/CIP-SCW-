...
(defun create-timber-panel (pt1 pt2 width h tf / mid-pt dx dy length panel-length panel-pt1 panel-pt2 panel-pt3 panel-pt4 pline-ent perp-dx perp-dy half-h flange-top-y panel-height timber-obj doc mspace hatch-obj sa)
  ...
  (if pline-ent
    (progn
      (setq timber-obj (vlax-ename->vla-object pline-ent))
...
(defun place-hpile-timber-along-boundary (boundary-ent hpile-spec ctc timber-thickness boundary-orient / 
  ...
  (princ (strcat "\n경계선 따라 배치 시작..."))
  (princ (strcat "\nH-Pile: H=" (rtos h 2 0) " B=" (rtos b 2 0) " tf=" (rtos tf 2 0)))
  (princ (strcat "\nC.T.C: " (rtos (* ctc 1000) 2 0) "mm"))
  (princ (strcat "\n토류판 두께: " (rtos timber-thickness 2 0) "mm"))
  
  ;; 반값 계산
  (setq half-h (/ h 2.0))
  
  ;; 토류판 블록 생성
  (setq timber-width (- (* ctc 1000) 50))  ; C.T.C - 50mm
  ...
  (princ (strcat "\nH-Pile Y 오프셋: +" (rtos hpile-offset 2 2) "mm (하단이 경계선)"))
  (princ (strcat "\n토류판 Y 오프셋: +" (rtos timber-offset 2 2) "mm (하단이 플랜지 상단)"))
  ...
  (while (< i num-segments)
    ...
    (while (< j (length positions))
      ...
      (setq pos (polar boundary-pt perp-angle timber-offset))
      (command "._INSERT" 
        timber-block
        pos
        1 1
      ...
      ;; H-Pile 좌측: 토류판 중심에서 좌측으로 timber-width/2, 수직으로 (hpile-offset - timber-offset)
      ...
      (foreach placed-pt placed-hpiles
        (if (< (distance hpile-left placed-pt) 10)
          (setq dist-ok nil)
        )
      )
      (if dist-ok
        (progn
          (command "._INSERT" hpile-block hpile-left 1 1 (/ (* seg-angle 180) pi))
          (setq placed-hpiles (append placed-hpiles (list hpile-left)))
        )
      )
      
      ;; H-Pile 우측: 토류판 중심에서 우측으로 timber-width/2, 수직으로 (hpile-offset - timber-offset)
      ...
      (if dist-ok
        (progn
          (command "._INSERT" hpile-block hpile-right 1 1 (/ (* seg-angle 180) pi))
...
(defun calculate-positions-on-segment (pt1 pt2 ctc / seg-length seg-mid positions 
  num-left num-right i dist pos seg-angle)
